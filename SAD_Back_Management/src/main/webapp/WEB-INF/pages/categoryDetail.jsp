...
<div class="row">
    <div id="breadcrumb" class="col-xs-12">
        <ol class="breadcrumb pull-left">
            ...
            <li><a href="./listFood">Category list</a></li>
    ...
    <div class="col-xs-12 col-sm-8 col-sm-offset-2">
        <div class="box">
            <div class="box-content">
                <c:set var="extraFoodGroupDetail" value="${categoryDetail}" scope="request"/>
                <c:set var="viewMode" value="${viewMode}" scope="request"/>

                <c:choose>
                    <c:when test="${viewMode =='update' }">
                        <c:set var="title" value="Update Category" scope="request"/>
                        <c:set var="method" value="update_category" scope="request"/>
                    </c:when>
                    <c:when test="${viewMode =='add' }">
                        <c:set var="title" value="Add New Category" scope="request"/>
                        <c:set var="method" value="add_category" scope="request"/>
                    </c:when>
                    <c:otherwise>
                        <c:set var="title" value="View Category" scope="request"/>
