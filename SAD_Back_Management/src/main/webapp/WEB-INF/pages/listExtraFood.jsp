...
<div class="row">
    <div id="breadcrumb" class="col-xs-12">
        <ol class="breadcrumb pull-left">
            ...
            <li><a href="./listExtraFood">List Exta Food</a></li>
        </ol>
    </div>
</div>
<div class="row">
    <div class="form-group has-warning has-feedback">
        <div class="col-sm-2">
            <a href="./addExtraFoodView" class="btn btn-primary btn-label-left">
                Add extra food
            </a>
        </div>

        <form method="get" id="searchForm" action="extraFoodDetail/search">
            ...
            <div class="col-sm-3">
                <select class="form-control placeholder" name="group" style="width: 100%">
                    <option value="name"
                    ${category == "name" ? 'selected="selected"' : ''}>Extra food name
                    </option>
                    <option value="group"
                    ${category == "group" ? 'selected="selected"' : ''}>Extra food name group
                    </option>
                </select>
            </div>
            <div class="col-sm-1">
        ...
        <div class="box">
            ...
            <div class="box-content no-padding">
                <table class="table table-bordered table-striped table-hover table-heading"
                    ...
                    <tbody>
                    <c:forEach begin="0" items="${listExtraFood}" var="extraFoodItem" varStatus="index">
                        <tr>
                            <td>${index.index +1}</td>
                            <td>
                                <a class="ajax-link"
                                   href="./extraGroupDetail?id=${extraFoodItem.id}"> ${extraFoodItem.name}
                                </a>
                            </td>
                            <td>
                                <a href="./extraGroupDetail?id=${extraFoodItem.groupId}">${extraFoodItem.groupName}</a>
                            </td>
                            <td><fmt:formatNumber type="number" pattern="#,###"
                                                  value="${extraFoodItem.price}"></fmt:formatNumber></td>
                            <td>
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <li><a href="./updateExtraFoodView?id=${extraFoodItem.id}">Update</a></li>
...
<script>
    ...
    callbackFunction = function () {
        $.ajax({
                    ...
                    url: window.location.origin + "/sad-bom/extraFood/delete_extra_food",
                    async: false,
                    success: function (data) {
                        if (data.statusCode == 0) {
                            ...
                            $("#popupMessage").text("Delete extra food is failed !");
                            openPopup(false);
                        }
                    }
                }
